...
const CaptureModal: React.FC<CaptureModalProps> = ({ locale, onClose, onAddTask, onConnectAi }) => {
  ...
  const selectFile = async (file?: File) => {
    ...
    if (file.size > 100_000) { setError(zh ? '当前支持 100KB 以内的文本文件。' : 'Text files must be under 100KB.'); return; }
  ...
  return (
    <div className="fixed inset-0 z-[240] bg-slate-950/45 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white w-full max-w-2xl rounded-[2rem] shadow-2xl p-7 md:p-9">
        ...
        <p className="mt-3 text-sm text-slate-500">{zh ? '粘贴会议记录、聊天摘要或想法，也可以选择 .txt / .md 文件。' : 'Paste meeting notes, a chat summary or an idea, or choose a .txt / .md file.'}</p>
        ...
        <div className="mt-4 flex flex-wrap items-center gap-3">
          <label className="cursor-pointer px-4 py-2.5 rounded-xl border border-slate-200 text-sm font-bold text-slate-700 hover:bg-slate-50"><input type="file" accept=".txt,.md,text/plain,text/markdown" className="hidden" onChange={(e) => selectFile(e.target.files?.[0])} />{zh ? '选择文本文件' : 'Choose text file'}</label>
