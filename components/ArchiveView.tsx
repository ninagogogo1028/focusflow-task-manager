  ...
  return (
    <div className="space-y-6">
      <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm">
        <h2 className="text-xl font-bold mb-6 text-slate-800">{t('archiveTitle')}</h2>
        <div className="space-y-4">
            ...
            <div key={task.id} className="flex flex-col md:flex-row items-start md:items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-200 opacity-75 hover:opacity-100 transition-opacity gap-4 md:gap-0">
              <div className="flex items-center gap-4 w-full md:w-auto">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-slate-500">{locale === 'zh-CN' ? '完成于' : 'Completed'} {new Date(task.createdAt).toLocaleDateString(locale)}</p>
                  {task.archivedAt && !task.isPermanent && (
                    <p className="text-[10px] text-slate-400 mt-0.5">
                      {t('daysLeft', { count: Math.max(0, 7 - Math.floor((Date.now() - task.archivedAt) / (24*60*60*1000))) })}
                    </p>
                  )}
                  {task.isPermanent && (
                    <p className="text-[10px] text-indigo-600 mt-0.5 font-bold">{t('keep')}</p>
                  )}
                </div>
              </div>
              <div className="flex items-center gap-3 w-full md:w-auto justify-end">
                ...
                >
                  {task.isPermanent ? t('unkeep') : t('keep')}
                </button>
                <button 
          ...
          {tasks.length === 0 && (
            <div className="text-center py-20 bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200">
              <p className="text-slate-400 font-medium">{t('archiveEmpty')}</p>
            </div>
          )}
        </div>
      </div>
